...
my_output <- read.csv("/Users/Cori/Research/Forests_on_the_Edge/URF 2018 Butkiewicz/Project_Output/v4/output_runs_v4.csv") #Reads in dataframe with pft, agb, density, dbh, basal area, etc. 
my_output <- merge(my_output, FRI)

# Edit the datatable
RUNID <- as.character(my_output$RUNID)
...
my_output$SLXSAND <- car::recode(my_output$Soil, "'s1'='0.93'; 's2'='0.8'; 's3'='0.66'; 's4'='0.52'; 's5'='0.38'")
my_output$SM_FIRE <- car::recode(my_output$Fire, "'1'='0.04'; '2'='0.03'; '3'='0.02'; '4'='0.01'; '5'='0'")
# my_output <- my_output[,-2]

# my_output.ba <- read.csv("/Users/Cori/Research/Forests_on_the_Edge/URF 2018 Butkiewicz/Project_Output/v4/output_batables_v4.csv")
# my_output.ba <- merge(my_output.ba,FRI)
# my_output.ba <- cbind(factors,my_output.ba)
# my_output.ba$Soil <- car::recode(my_output.ba$Soil, "'s1'='0.93'; 's2'='0.8'; 's3'='0.66'; 's4'='0.52'; 's5'='0.38'")
# my_output.ba <- my_output.ba[,-2]
# 
...
pdf("/Users/Cori/Research/Forests_on_the_Edge/URF 2018 Butkiewicz/Project_Output/v4/soil_moisture_all.pdf")
ggplot(my_output,aes(x = year, y = soil_moist)) +
  geom_line() +
  xlab("Years") +
  ylab("Soil Moisture") + 
  facet_grid(SLXSAND ~ SM_FIRE) + 
  # geom_vline(aes(xintercept = which(my_output$fire="Yes"))) +
...
# ----------------------------
pdf("/Users/Cori/Research/Forests_on_the_Edge/URF 2018 Butkiewicz/Project_Output/v4/density.pdf")
ggplot(my_output,aes(x = year, y = p.dens.tree)) +
  geom_line(color='dark green') +
  xlab("Years") +
  ylab("Density") + 
  facet_grid(SLXSAND~SM_FIRE) + 
  # geom_vline(aes(xintercept = which(my_output$fire="Yes"))) +
  ggtitle("Tree Density (>10 cm DBH)") +
  mytheme
dev.off()
...
# --------------------------------
pdf("/Users/Cori/Research/Forests_on_the_Edge/URF 2018 Butkiewicz/Project_Output/v4/basal_area.pdf")
ggplot(my_output,aes(x = year, y = p.ba)) +
...
# -------------------------------
# CHANGE IN DBH OVER TIME
# -------------------------------

pdf("/Users/Cori/Research/Forests_on_the_Edge/URF 2018 Butkiewicz/Project_Output/v4/dbh.pdf")
ggplot(my_output,aes(x = year, y = p.dbh)) +
  geom_line(color='blue') +
  xlab("Years") +
  ylab("DBH, cm") + 
  facet_grid(SLXSAND~SM_FIRE) + 
  # geom_vline(aes(xintercept = which(my_output$fire="Yes"))) +
  ggtitle("DBH of all Trees") +
  mytheme
dev.off()

pdf("/Users/Cori/Research/Forests_on_the_Edge/URF 2018 Butkiewicz/Project_Output/v4/dbh_tree.pdf")
ggplot(my_output,aes(x = year, y = p.dbh.tree)) +
  geom_line(color='dark blue') +
  xlab("Years") +
  ylab("DBH, cm") + 
  facet_grid(SLXSAND~SM_FIRE) + 
  # geom_vline(aes(xintercept = which(my_output$fire="Yes"))) +
  ggtitle("DBH of trees >10 cm") +
  mytheme
dev.off

# --------------------------

# Interesting to look at the table in this way: 
my_output.test <- subset(my_output, subset=my_output$year==101)
test <- aggregate(my_output.test$p.dbh, by=list(my_output.test[,"FRI"]), FUN=mean)
colnames(test) <- c("FRI","dbh")
temp <- aggregate(my_output.test$p.dens.tree, by=list(my_output.test[,"FRI"]), FUN=mean)
test$p.dens.tree <- temp[,2]
temp <- aggregate(my_output.test$p.ba, by=list(my_output.test[,"FRI"]), FUN=mean)
test$p.ba <- temp[,2]

pdf("/Users/Cori/Research/Forests_on_the_Edge/URF 2018 Butkiewicz/Project_Output/v4/FRI_scatter")
ggplot(test, aes(x=FRI, y=p.dens.tree)) + 
  geom_point(color="dark orange") +
  xlab("Fire Return Inverval") + 
  ylab("Tree Density") + 
  mytheme
dev.off

test.cub <- test[1:9,]
ggplot(test.cub, aes(x=FRI, y=dens.tree)) + 
  geom_point() +
  xlab("Fire Return Inverval") + 
  ylab("Tree Density") + 
  mytheme
...
# ----------
# DOES DENSITY CORRELATE WITH SOIL MOISTURE?
# ----------

pdf("/Users/Cori/Research/Forests_on_the_Edge/URF 2018 Butkiewicz/Project_Output/v4/SM_scatter")
ggplot(my_output, aes(x=soil_moist, y=p.dens.tree)) + 
  geom_point() +
  xlab("Soil Moisture") + 
  ylab("Tree Density") + 
  mytheme
